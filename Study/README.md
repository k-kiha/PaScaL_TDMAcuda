# PaScaL_TDMAcuda Study workflow
...
```text
Makefile                         build the two profile drivers
example_fortran_profile.f90      CUDA Fortran profile driver
example_cuda_cxx_profile.cu      CUDA C++ profile driver
run_study_sweep.sh               case generation and execution
...
```text
Study/example_fortran_profile
Study/example_cuda_cxx_profile
```

From `Study/`, the component-level equivalent is:
...
```bash
mpirun -np 4 ./Study/example_cuda_cxx_profile 64 64 2048 10 128 128
mpirun -np 4 ./Study/example_fortran_profile 64 64 2048 10 128 128
```
...
```bash
STUDY_PRESET=quick ./Study/run_study_sweep.sh
```
...
```bash
DRY_RUN=1 STUDY_PRESET=quick ./Study/run_study_sweep.sh
```
...
By default, a new run writes timestamped files directly under `Study/`:
...
`Study/result/`. They are not automatically replaced by a new sweep.
...
`make veryclean` removes the two Study executables but preserves CSV files and
the curated `result/` directory. The repository helper
`scripts/clean_for_sync.sh` provides a wider dry-run cleanup preview.
