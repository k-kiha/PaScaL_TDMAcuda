...
```bash
PASCAL_TDMA_MPI_MODE=host mpirun -np 4 ./Study/example_cuda_cxx_profile 64 64 2048 10
mpirun -np 4 ./Study/example_fortran_profile 64 64 2048 10
```

The Fortran driver reports total solver time. The CUDA C++ port also has a
more detailed breakdown under `CUDA_CXX_Port/examples/ex_tdma_profile.cu`.
...
Study CSV files are written directly under `Study/`.
